...
for dataset in datasets:
    ...
    bandwidth = estimate_bandwidth(dataset, quantile=.2)
    meanshift = MeanShift(bandwidth=bandwidth)
    meanshift.fit_predict(dataset)
    axs[1, 0].scatter(dataset[:, 0], dataset[:, 1], c=meanshift.labels_)
    validation['Silhouette Coefficient'].append(metrics.silhouette_score(dataset, meanshift.labels_, metric='euclidean'))
